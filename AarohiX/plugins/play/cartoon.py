...
@app.on_message(
    filters.command(["✨ كرتون"])
async def handle_aflam_command(client: Client, message: Message):
    global mid
    mid = message.message_id
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton("انمي 🧚‍♀️", callback_data="anmie2 " + str(message.from_user.id))],
        [InlineKeyboardButton("مسلسلات كارتون 👼", callback_data="fmoslsl " + str(message.from_user.id))],
        [InlineKeyboardButton("افلام كارتون 🍿", callback_data="fcartoon " + str(message.from_user.id))],
        [InlineKeyboardButton("ضيـف البـوت لمجمـوعتـك ✅", url=f"https://t.me/{get_bot_information()[1]}?startgroup=new")],
    ])
    await message.reply_text("◍ أهلا بيك بقائمه افلام ومسلسلات الكارتون والانمي\n√", reply_markup=keyboard)
