...
async def handle_aflam_command(client: Client, message: Message):
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton("مسلسلات 📼", callback_data="moslsl " + str(message.from_user.id))],
            [InlineKeyboardButton("مسرحيات 🎭", callback_data="msrahia " + str(message.from_user.id))],
            [InlineKeyboardButton("⌞ ѕᴏụʀᴄᴇ ᴀʟᴍᴏʀᴛᴀɢᴇʟ ⌝⚡", url="https://t.me/AlmortagelTech")],
        ]
    )
    await message.reply_text("◍ اهلا بيك في قائمة الافلام والمسلسلات العربيه\n√", reply_markup=keyboard)
